...
_SESSION_STORE_DIR: Final[Path] = _REPO_ROOT / ".acp-sessions" / "workspace-agent"
_DEFAULT_CODEX_MODEL_NAME: Final[str] = "gpt-5.4"
_CODEX_MODEL_LABELS: Final[dict[str, str]] = {
    "gpt-5.4": "gpt-5.4",
...
_SUPPORTED_CODEX_MODELS: Final[tuple[str, ...]] = tuple(_CODEX_MODEL_LABELS)
...
def _env_model_name() -> str | None:
    ...
    if not configured_model:
        return None
    return configured_model.removeprefix("codex:").strip() or None


def _default_model_name() -> str:
    return _env_model_name() or _DEFAULT_CODEX_MODEL_NAME


def _codex_model_id(model_name: str) -> str:
    return f"codex:{model_name}"


@cache
def _build_codex_model(model_name: str) -> CodexResponsesModel:
    return create_codex_responses_model(model_name)


def _available_models() -> list[AdapterModel]:
    model_names = list(_SUPPORTED_CODEX_MODELS)
    env_model_name = _env_model_name()
    if env_model_name is not None and env_model_name not in model_names:
        model_names.insert(0, env_model_name)
    elif env_model_name is not None and env_model_name in model_names:
        model_names.remove(env_model_name)
        model_names.insert(0, env_model_name)
    return [
        AdapterModel(
            model_id=_codex_model_id(model_name),
            name=_CODEX_MODEL_LABELS.get(model_name, model_name),
            description=f"Codex-backed Responses model `{model_name}`.",
            override=_build_codex_model(model_name),
        )
...
class WorkspaceAgentSource:
    ...
    async def get_agent(
        ...
        agent: Agent[None, str | DeferredToolRequests] = Agent(  # pyright: ignore[reportCallIssue]
            _build_codex_model(_default_model_name()),
