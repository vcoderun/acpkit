...
Both model factories support incremental text delivery. Pydantic AI's
non-streaming `request()` still consumes the required Responses SSE stream and
returns the final response; `request_stream()` exposes deltas as they arrive.

```python
async with agent.run_stream("Explain the change briefly.") as result:
    async for text in result.stream_text(delta=True):
        print(text, end="", flush=True)
```

For LangChain, use the chat model's streaming API:
...
async for chunk in model.astream("Explain the change briefly."):
    print(chunk.content, end="", flush=True)
```

The LangChain factory accepts `streaming=False` for consumers that want a final
message. This does not disable the Codex backend's streamed wire protocol.
